...
map<double, multiset<double>, compareDouble> XgivenY;
map<double, multiset<double>, compareDouble> YgivenX;
...
void erase(const point &p) {
    // printf("Erasing (%d, %d)\n", original[p].first, original[p].second);
    multiset<double>::iterator k;
...
//    D(jumps);
    for(int i = 0; i < jumps; i++){
        ...
        if(moves[i] == 'A'){
            const multiset<double> &p = YgivenX[ current.x ];
        ...
        if(moves[i] == 'B'){
            const multiset<double> &p = XgivenY[ current.y ];
