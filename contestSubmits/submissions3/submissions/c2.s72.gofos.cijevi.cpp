...
bool reach(int i, int j, int dir, int seenNormal, int seenCrosses) {
    ...
    if (mat[i][j] == 'Z') {
        // D(mustSeeNormal);
        // D(mustSeeCrosses);
        if (mustSeeNormal == seenNormal and 2 * mustSeeCrosses == seenCrosses) return true;
        return false;
    }
...
bool check(pair<int, int> start) {
    ...
    for (int k = 0; k < 4; ++k) {
        ...
        if (mat[ni][nj] != '.' and mat[ni][nj] != 'Z') return reach(ni, nj, k, 0, 0);
    }
    assert(false);
}
...
int main(){
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            ...
            crosses += (mat[i][j] == '+');
            if (mat[i][j] != '+' and mat[i][j] != '.' and mat[i][j] != 'Z' and mat[i][j] != 'M') {
                normal++;
            }
        }
    }
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            ...
            for (int k = 0; k < 7; ++k) {
                mat[i][j] = options[k];
                if (options[k] == '+') mustSeeCrosses++;
                if (options[k] != '+') mustSeeNormal++;
